...
class SWAGInference(object):
    ...
    def __init__(
        ...
        self.theta2bar = self._create_weight_copy()

        # Full SWAG
        # TODO(2): create attributes for SWAG-diagonal
        #  Hint: check collections.deque
        # torch.zeros_like((0,param.data.numel()), requires_grad=False)
        self.d_hat = {name: collections.deque() for name, param in self.network.named_parameters()}

        # Calibration, prediction, and other attributes
        # TODO(2): create additional attributes, e.g., for calibration
        self._prediction_threshold = None  # this is an example, feel free to be creative

    def update_swag(self, num_of_models = 1) -> None:
        """
        ...
        for name, param in current_params.items():
            ...
            self.theta_bar[name] = (self.theta_bar[name] * num_of_models + param) / (num_of_models + 1)
            self.theta2bar[name] = (self.theta2bar[name] * num_of_models + param ** 2) / (num_of_models + 1)

        # Full SWAG
        if self.inference_mode == InferenceMode.SWAG_FULL:
            # TODO(2): update full SWAG attributes for weight `name` using `current_params` and `param`
            for name, param in current_params.items():
                if num_of_models > self.deviation_matrix_max_rank:
                    self.d_hat[name].popleft()
                d = (param-self.theta_bar[name]).view(-1, 1).t()
                self.d_hat[name].append(d)
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        lr_scheduler = SWAGScheduler(
            ...
            initial_lr=self.swag_learning_rate,
            final_lr=0.0001
        )
        ...
        current_params = {name: param.detach() for name, param in self.network.named_parameters()}
        for name, param in current_params.items():
            self.theta_bar[name] = param
            self.theta2bar[name] = param ** 2
        ...
        with tqdm.trange(self.swag_epochs, desc="Running gradient descent for SWA") as pbar:
            ...
            for epoch in pbar:
                ...
                if (epoch + 1) % self.swag_update_freq == 0:
                    ...
                    print("Updating SWAG statistics...", "n = ", n)
                    self.update_swag(n)
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        for _ in tqdm.trange(self.bma_samples, desc="Performing Bayesian model averaging"):
            ...
            for (batch_xs,) in loader:
                ...
                # softmax?
                # predictions = torch.softmax(batch_ys, dim=-1)
                predictions = batch_ys
                model_sample_predictions.append(predictions)
            # Stack predictions for the current model sample
            model_sample_predictions = torch.cat(model_sample_predictions)
            per_model_sample_predictions.append(model_sample_predictions.softmax(dim=-1))
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_std = torch.sqrt(torch.clamp(self.theta2bar[name] - current_mean ** 2,1e-30))
            assert current_mean.size() == param.size() and current_std.size() == param.size()

            # Diagonal part
            sampled_param = current_mean + current_std * z_1
            # Only use mean to sample the model
            # sampled_param = current_mean

            # Full SWAG part
            if self.inference_mode == InferenceMode.SWAG_FULL:
                # TODO(2): Sample parameter values for full SWAG
                current_d_hat = torch.stack([x.squeeze() for x in self.d_hat[name]],dim=0)
                # print(current_d_hat.size())
                scale_1 = (1/2)**0.5
                scale_2 = scale_1 /((self.deviation_matrix_max_rank-1) **0.5)
                z_2 = torch.randn(current_d_hat.size()[0],1)
                # print(z_2.size())
                sampled_param = current_mean + scale_1*current_std * z_1
                sampled_param += scale_2*current_d_hat.t().matmul(z_2).view_as(current_mean)
...
class SWAGScheduler(torch.optim.lr_scheduler.LRScheduler):
    ...
    def calculate_lr(self, current_epoch: float, old_lr: float) -> float:
        ...
        # return old_lr
        step = round(current_epoch * self.steps_per_epoch)
        lr = self.final_lr + (self.initial_lr - self.final_lr) * (1 - step / self.total_steps)
        # print("lr = ", lr)
        # return lr
        return old_lr
